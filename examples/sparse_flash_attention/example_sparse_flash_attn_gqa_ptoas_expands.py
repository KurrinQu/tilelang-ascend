...
def sparse_attention_fwd(
    ...
    workspace_1_shape = [workspace_block_num * 2, BI, D]
    workspace_2_shape = [workspace_block_num * 2, H_per_block, BI]
    workspace_4_shape = [workspace_block_num * 2, H_per_block, D]
    ...
    def main(
        ...
        with T.Kernel(seq_len * REPLICATE_H * batch * kv_group, is_npu=True) as (cid, vid):
            ...
            with T.Scope("C"):
                ...
                # T.barrier_all()
                for i in T.serial(NI + 2):
                    if i >= 1 and i < NI + 1:
                        i_i = i - 1
                        pingpang = i_i % 2
                        T.wait_cross_flag(0, "MTE2")
                        # T.barrier_all()
                        T.copy(workspace_1[cid + (workspace_block_num * pingpang), 0:BI, 0:D], kv_l1)
                        # T.barrier_all()
                        T.set_flag("MTE2", "M", 1)
                    ...
                    if i >= 2 and i < NI + 2:
                        ...
                        T.copy(workspace_3[cid + (workspace_block_num * pingpang), 0:H_per_block, 0:BI], acc_s_l1)
                        # T.barrier_all()
                        T.set_flag("MTE2", "M", 3)
                        ...
                        T.copy(acc_o_l0c, workspace_4[cid + (workspace_block_num * pingpang), 0:H_per_block, 0:D])
                        # T.barrier_all()
                ...
                for i in range(NI + 2):
                    if i >= 0 and i < NI:
                        ...
                        for bi_i in range(BI // 2):
                            ...
                            T.copy(kv_ub, workspace_1[cid + (workspace_block_num * pingpang), bi_i + vid * BI // 2, :])
                            # T.barrier_all()
                    ...
                    if i >= 1 and i < NI + 1:
                        ...
                        T.copy(
                            acc_s_half, workspace_3[cid + (workspace_block_num * pingpang),
                    ...
                    if i >= 2 and i < NI + 2:
                        ...
                        T.copy(
                            workspace_4[cid + (workspace_block_num * pingpang), vid * v_block:vid * v_block + v_block, :],
                            acc_o_ub)
                        # T.barrier_all()
                        T.set_flag("MTE2", "V", 7)
